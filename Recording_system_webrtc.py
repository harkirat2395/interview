...
class RecordingSystemWebRTC:
    ...
    def record_continuous_interview_webrtc(self, questions_list, duration_per_question, ui_callbacks):
            ...
            try:
                ...
                st.error("❌ Audio recorder not installed. Run: pip install audio-recorder-streamlit")
                return {"error": "Missing audio recorder"}
            ...
            # Create a unique key for setup
            setup_key = f"setup_phase_{int(time.time())}"
            
            # WebRTC streamer for setup with CORRECTED media constraints
            ...
            # Wait for WebRTC to start
            if not setup_ctx.state.playing:
                st.warning("⏳ Please click 'START' to begin camera setup")
            ...
            while not setup_complete and (time.time() - setup_start) < setup_timeout:
                if setup_ctx.state.playing and processor.is_setup_complete():
                    setup_complete = True
            ...
            if not setup_complete:
                st.error("❌ Setup timeout - Please refresh and try again")
                return {"error": "Setup phase failed or timeout"}
            
            processor.end_setup_phase()
            
            # Stop setup stream
            if setup_ctx.state.playing:
                st.info("✅ Setup complete! Starting interview in 5 seconds...")
                time.sleep(5)
            ...
            **Starting in 5 seconds...**
            ...
            for q_idx, question_data in enumerate(questions_list):
                ...
                for i in range(3, 0, -1):
                    countdown_placeholder.info(f"⏱️ Starting in {i}s...")
                    time.sleep(1)
                ...
                question_key = f"question_{q_idx}_{int(time.time())}"
                
                # WebRTC streamer for this question with BOTH video and audio
                ...
                if not question_ctx.state.playing:
                    st.warning("⏳ Please click 'START' to begin recording")
                    time.sleep(2)
                ...
                if AUDIO_RECORDER_AVAILABLE:
                    audio_bytes = audio_recorder(
                        ...
                        icon_size="2x",
                        key=f"audio_{q_idx}_{int(time.time())}"
                    )
                ...
                while (time.time() - question_start) < duration_per_question:
                    ...
                    if question_ctx.state.playing and processor.violation_detected:
                        violation_reason = processor.violation_reason
                        violation_frame = processor.violation_frame
                    ...
                    # Update status
                    if question_ctx.state.playing:
                        eye_contact = processor.eye_contact_frames
                        total = max(processor.total_frames, 1)
                        ...
                        **Status:**
                        ...
                        - 💡 Lighting: {processor.lighting_status}
                        - ⚠️ Attention: {processor.attention_status}
                        """)
            ...
            if total_violations > 0:
                st.warning(f"⚠️ **INTERVIEW COMPLETED WITH {total_violations} VIOLATION(S)**")
            else:
